...
def show_merge_preview():
    ...
    if approved.empty:
        print("No AI rows marked ready_to_merge = Yes.")
        return

    master_lookup = master.set_index("business_id").to_dict("index")

    print("=" * 80)
    print("AI Enrichment Merge Preview")
    print("=" * 80)
    ...
    for _, row in approved.iterrows():
        ...
        print("Current description:", master_row.get("post_content", ""))
        print("Suggested description:", row.get("suggested_post_content", ""))
        print("Suggested SEO title:", row.get("suggested_seo_directory_title", ""))
        print("Suggested meta:", row.get("suggested_seo_meta_description", ""))
        print("Suggested keywords:", row.get("suggested_searchable_keywords", ""))
        print("Confidence:", row.get("confidence", ""))
        print("Notes:", row.get("notes", ""))

    print("=" * 80)
    print(f"Approved AI rows ready to merge: {len(approved)}")
